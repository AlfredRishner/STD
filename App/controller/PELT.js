Ext.define('proyecto.controller.PELT', {
    ...
    stores: ['PELTStore'],
    models: ['PELTModel'],
    views: ['documentos.gridPELT', 'documentos.formPELT', 'documentos.winPELT', 'documentos.panelPELT', 'documentos.gridArchivos', 'documentos.winArchivo', 'documentos.formArchivo'/*'mesa.ComboMesEjecucion'*/],
    refs: [
        {
            ref: 'gridPELT',
            selector: ['viewport > gridPELT', 'viewport > gridArchivos']
        }],
    init: function () {
        this.control({
            ...
            'gridPrincipal button[action=nuevo]': {
            ...
            'gridArchivos button[action=VentanaSubirArch]': {
                // 
                click: this.ventana
            },
            'formArchivo  button[action=subirarchivo]': {
    ...
    SubirArchivo: function (btn) {
        //  alert('llego');
        var me = this,
                form = btn.up('form'),
                win = form.up('winArchivo'),
                basicForm = form.getForm(),
                // store = this.getStore('FebreroStore'), //grid.getStore(),
                record = basicForm.getRecord(),
                values = basicForm.getValues();
        ...
        {
            basicForm.submit({
                                        url: 'index.php/SubirArchivo/subirArchivo',
                                        waitMsg: 'Loading...',
    ...
    ventana: function (grid, record) {
        //alert('ok');
        var view = Ext.widget('winArchivo');
        /*  if (record) {
