Ext.define('proyecto.controller.Setiembre', {
    extend: 'Ext.app.Controller',
    stores: ['SetiembreStore','StoreComboGrupo'],
    models: ['EjecucionModel'],
    views: ['setiembre.gridSetiembre', 'setiembre.formSetiembre', 'setiembre.winSetiembre'],
    refs: [
        {
            ref: 'list',
            selector: 'gridSetiembre'
        }],
    init: function() {
        this.control({
            'gridSetiembre dataview': {
                itemdblclick: this.add
            },
            'gridSetiembre button[action=nuevo]': {
                //click: this.editarContato
                click: this.add
            },
            'gridSetiembre button[action=eliminar]': {
                click: this.eliminarPepeleta
            },
            'formSetiembre button[action=guardarEjecucion]': {
                //click: this.updateContato
                click: this.guardarEjecucion
            },
            'gridSetiembre button[action=printerDiario]': {
                click: this.repoterDiario
            }
            ,
            'gridSetiembre button[action=modificar]': {
                click: this.repoterDiario
            }
            ,
            'gridSetiembre button[action=printerEjec]': {
    ...
    guardarnuevo: function(btn) {
        var me = this,
                form = btn.up('formSetiembre'),
                win = form.up('window'),
                basicForm = form.getForm(),
                store = this.getStore('SetiembreStore'), //grid.getStore(),
        ...
        if (basicForm.isValid())
        {
            ...
            record.set(values);
            this.getStore('SetiembreStore').add(record);
            store.sync();
            win.close();
            this.getStore('SetiembreStore').load();
    ...
    add: function(grid, record) {
        var view = Ext.widget('winSetiembre');
        if (record) {
	        ...
	        success: function(result,request) {
		        ...
		        if(json.success){
		        	Ext.example.msg('Mensaje','Rreporte Generado!');
                            ...
                            this.imprimirDocumentosEnviados('ReporteEject', 'mes=Setiembre&nombremeta='+json.registros[0].nombre+'&idmeta='+json.registros[0].idmeta+'&meta='+json.registros[0].codigo+'&idmes=9', 'Impresion de Reporte');
    ...
    eliminarPepeleta: function(button) {
        ...
        if (selection)
        {
            if (selection.get('firmapersonal') == '1' || selection.get('firmaadministracion') == '1' || selection.get('firmajefeinmediato') == '1')
                ...
                var store = this.getStore('SetiembreStore');
