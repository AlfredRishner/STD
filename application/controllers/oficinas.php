...
class oficinas extends CI_Controller
{
    ...
    public function __construct()
    {
     ...
     //   $this->load->database('default');
        ...
        function listado(){
             $this->load->model('oficinas_model','', TRUE);
	     ...
	     //$limit = isset($this->input->post('limit'))?$this->input->post('limit'):5; //número de registros a 
             ...
             $this->oficinas_model->listado($start,$limit);
        }
        function crear(){
             $this->load->model('oficinas_model','', TRUE);
              $registros= $this->input->post('registros');
             // $idmeta=$this->session->userdata('idmeta');
             $this->oficinas_model->crear($registros);
        }
        function eliminar(){
             $this->load->model('oficinas_model','', TRUE);
             $registros= $this->input->post('registros');
             // $idmeta=$this->session->userdata('idmeta');
             $this->oficinas_model->eliminar($registros);
        }
        function actualiza(){
             $this->load->model('oficinas_model','', TRUE);
             //$node= $this->input->get('node');
             $registros= $this->input->post('registros');
             //$idmeta=$this->session->userdata('idmeta');
             $this->oficinas_model->actualiza($registros);
        }
