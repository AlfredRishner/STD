     ...
     //   $this->load->database('default');
        ...
        function listado(){
	     ...
	     //$limit = isset($this->input->post('limit'))?$this->input->post('limit'):5; //número de registros a 
             ...
             $this->documentos_model->listado($start,$limit);
        }
        function crear(){
             $this->load->model('documentos_model','', TRUE);
              $registros= $this->input->post('registros');
             // $idmeta=$this->session->userdata('idmeta');
             $this->documentos_model->crear($registros);
        }
        function eliminar(){
             $this->load->model('documentos_model','', TRUE);
             $registros= $this->input->post('registros');
             // $idmeta=$this->session->userdata('idmeta');
             $this->documentos_model->eliminar($registros);
        }
        function actualiza(){
             $this->load->model('documentos_model','', TRUE);
             //$node= $this->input->get('node');
             $registros= $this->input->post('registros');
             //$idmeta=$this->session->userdata('idmeta');
             $this->documentos_model->actualiza($registros);
        }
